...
//  ffdec_mp3  = gst_element_factory_make ("ffdec_mp3", "ffdec_mp3");
  flump3dec  = gst_element_factory_make ("flump3dec", "flump3dec");
...
//  if (!pipeline || !src || !ffdemux_mp3 || !ffdec_mp3 || !sink)
  if (!pipeline || !src || !flump3dec || !sink)
...
#else
  /* we add all elements into the pipeline */
  gst_bin_add_many (GST_BIN (pipeline), src, flump3dec, sink, NULL);
...
#else
  gst_element_link (src, flump3dec);
  gst_element_link (flump3dec, sink);
