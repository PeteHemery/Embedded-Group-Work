	  ...
	  if (mac_changed == TRUE)
	    {
	      ...
	      state = CREATEHEADERS;
	      opcode = PLAY;
	      strncpy(localData, closest_mac, MAC_LEN);
	      pthread_mutex_unlock(&network_Mutex);
	      
	      printf("Going to creat a packet to send a mac address :)\nreqCode:%c\nstate=%c\nopcode:%c\n", reqCode, state, opcode);
	      break;
	    }
